...
select rownum rn, board_idx, title, bcontent, 
(select count(*) from board_reply r where r.board_idx = b.board_idx) as rcnt
from (select rownum rn, b.* from board b) b;

select count(*) as rcnt, board_idx from board_reply r, (select * from board) b where r.board_idx=b.board_idx;
select * from board_reply;
...
select * from question order by qna_idx desc;
...
select * from answer;
create table answer(
answer_idx NUMBER,      /*답변시퀀스*/
awriter VARCHAR2(15),   /*답변작성자*/
acontent VARCHAR2(500), /*답변내용*/
alike NUMBER,           /*답변추천수*/
adate DATE,             /*답변한날짜*/
qna_idx NUMBER,         /*질문시퀀스*/
 PRIMARY KEY(answer_idx)
);
desc answer;
commit;
insert into answer values(answer_seq.nextval, 'user1','acontent', 0 , sysdate, '1');
commit;
delete from answer;
select count(*) as acnt from answer where qna_idx=1;